...
- `denoiser=fft3d|bilateral|motion`: クリーンなベース画像の生成方法を選択する。デフォルトは`fft3d`。`motion`は元フレームを保持し、動き補償した輝度のベース画像を生成して、元画像との差分からグレインを推定する。クロマには局所的なエッジ保持デノイズを使用する。
- `motion-refs=1|2`: デフォルトでは過去の参照フレームを2枚使用して時間方向の分離を強化し、`1`では処理負荷を下げる。`denoiser=motion`でのみ有効。デフォルトは`2`。
- `retain=auto|<float>`: 測定したグレイン残差の一部を輝度のエンコード画像に残し、合成強度をsqrt(1-retain^2)倍して再生時のグレイン分散を一致させる。`auto`は方向性のあるディテールを失う危険度に応じて0.0から0.5まで0.05刻みの値を選び、モデル窓内で平滑化する。数値指定では0.0から0.9までの割合を固定する (デフォルト 0.0)。残した残差はビットレートを消費するが、元位置のグレインと確実に分離できないディテールを保持する。合成グレインはビット消費ゼロだが位置はランダムとなる。
- `modelsrc=on|off`: ARグレインモデルをデノイズ残差ではなく元画像から推定する (デフォルト `off`)。残差はデノイザによって既に白色化されているため、そこから得られるモデルは実際のフィルムより細かいグレインを表す。元画像の同じ平坦ブロックから、ブロックごとの平均と平面成分のみを除去して推定することで空間的な相関を回復する。強度カーブは元画像の分散からベース画像の分散を差し引いて求めるため、デノイザが実際に除去した分のグレインのみを合成する。
