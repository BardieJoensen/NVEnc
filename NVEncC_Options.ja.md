...
- `retain=<float>`: 測定したグレイン残差のうち指定した割合を輝度のエンコード画像に残し、合成強度をsqrt(1-retain^2)倍して再生時のグレイン分散を一致させる (0.0 - 0.9、デフォルト 0.0)。残したグレインはビットレートを消費するが元の位置を保ち、合成グレインはビット消費ゼロだが位置はランダムとなる。
