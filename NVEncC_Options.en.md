...
- `modelsrc=on|off`: fit the AR grain model from the source rather than from the denoiser's residual (default `off`). The residual reaches the solver already whitened by the denoiser, so the model it produces describes grain that is finer than the film's; fitting instead from the same flat blocks of the source, with only each block's own mean-plus-plane removed, recovers the spatial structure. The strength curve is derived from the source variance minus the base's own, so the synthesis replaces only the grain the denoiser actually removed.
