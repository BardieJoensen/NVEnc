...
- `denoiser=fft3d|bilateral|motion`：选择干净基础画面的生成方式。默认为`fft3d`。`motion`会保留原始帧，生成运动补偿后的干净亮度基础画面，并从原始画面与基础画面的残差中估算颗粒；色度使用局部边缘保持降噪。
- `motion-refs=1|2`：默认使用两个过去参考帧以增强时间分离；设为`1`可降低处理开销。仅对`denoiser=motion`生效，默认值为`2`。
- `retain=auto|<float>`：在编码图像的亮度中保留部分测得的颗粒残差，并将合成强度缩放为sqrt(1-retain^2)，使播放时的颗粒方差保持一致。`auto`根据方向性细节受损风险，在0.0到0.5之间以0.05为步长选择数值，并在模型窗口内进行平滑；数值参数可将比例固定为0.0到0.9（默认0.0）。保留残差会消耗码率，但可保留原始位置的颗粒以及无法可靠分离的细节；合成颗粒不消耗码率，但位置随机。
- `modelsrc=on|off`：从源图像而非降噪残差拟合AR颗粒模型（默认`off`）。残差在到达求解器时已被降噪器白化，因此由其得到的模型所描述的颗粒比胶片实际的更细。改为从源图像的相同平坦块拟合，仅去除每个块自身的均值与平面分量，即可恢复空间相关性。强度曲线由源图像方差减去基础图像自身的方差得出，因此只合成降噪器实际去除的那部分颗粒。
